...
class SidebarComponent:
        ...
        if use_dynamic_withdrawal:
            ...
            if use_wage_based_savings:
                ...
                if savings_rate_mode == "Constant rate":
                    ...
                    # Don't pass value parameter when using key to avoid warnings
                ...
                # Initialize session state if not present to avoid warnings
                if "current_wage_input" not in st.session_state:
                    st.session_state.current_wage_input = "150000"
            ...
            # Initialize session state if not present to avoid warnings
            if "simple_spending_input" not in st.session_state:
                st.session_state.simple_spending_input = str(int(self.config.default_annual_spend))
    ...
    def create_portfolio_pie_chart(self, asset_weights: Dict[str, float], initial_balance: Optional[float] = None) -> go.Figure:
